 ...
 *
 * Options:
 *   - 'apiKey'          API key from APIs console if only using public profile data.
 *   - 'clientId'        Client ID from the APIs console, matches ID configured in G+ Sign-In Button
 *   - 'clientSecret'    Corresponding client secret if offline access requested
 *   - 'presenters'      Array of valid client IDs that are authorized presenters of ID Tokens
 *   - 'skipProfile'     True if user profile isn't required.
 *
 * @constructor
 * @param {Object} options configuration
 * @param {GooglePlusStrategyCallback} callback
 */
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  this.validateAccessToken(context, function(err, context) {
    if (err) {
      return done(err);
    }
    self.exchangeAuthorizationCode(context, function(err, context) {
      if (err) {
        return done(err);
      }
      self.validateIdToken(context, function(err, context) {
        if (err) {
          return done(err);
        }
        self.loadProfile(context, function(err, context) {
          if (err) {
            return done(err);
          }
...
Strategy.prototype.validateIdToken = function(context, callback) {
  ...
  this.keyManager.fetchKey(jwt.header.kid, function(err, key) {
    ...
    } else if (self.authorizedPresenters != null && self.authorizedPresenters.indexOf(jwt.payload.azp) == -1) {
